...
ARGV.each do |path|
  ...
  required = %w[title description permalink categories tags last_modified_at image image_alt image_source fact_check_status]
  ...
  unless %w[complete not-needed].include?(data["fact_check_status"].to_s)
